...
mod tests {
    ...
    fn test_watchers_hashmap_usage() {
        ...
        assert!(cnf.watchers.contains_key(&2));
        assert!(cnf.watchers.contains_key(&-1));
        assert!(cnf.watchers.contains_key(&3));
    }
    ...
    fn test_unit_clauses_cleared() {
        ...
        // (it's an internal state that gets cleared during processing)
        assert!(cnf.unit_clauses.is_empty());
    }
}
